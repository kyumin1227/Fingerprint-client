...
"color: rgb(227, 227, 227); border: 0px;")
		...
		self.delete_fingerprint_button.clicked.connect(lambda: self.close_door())
...
"font-size: 35px;")
	...
	def retranslateUi(self, MainWindow):
		...
		self.delete_fingerprint_button.setText(_translate("MainWindow", "문 닫기"))
	...
	def log(self, action):
		...
		if self.result[0] != -1 and self.result[1] >= 65:
			...
			res = requests.post(SERVER_URL + "/logs", data=json.dumps(log_dic), headers=headers)

			if res.status_code == 200:
				self.fp_label_text.setText(res.json()["message"])
				self.out_label_text.setText(res.json()["message"])
			else:
	...
	def data(self, action):
		...
		# 이미 지문이 등록된 학번인지 체크
		res = requests.get(SERVER_URL + "/students/" + self.stdNum)
		...
		if res.json()["success"] and action == "create":
			...
			while self.activate and time.time() - self.start_time < 5:
				if f.readImage() != False:
					...
					res = requests.post(f"{SERVER_URL}/students", data= json.dumps(new_fingerprint_dic), headers=headers)
		...
		# 지문 삭제
			...
			if self.result[0] != -1 and self.result[1] >= 65:
				...
				if self.del_stdNum == self.stdNum:
					res = requests.delete(SERVER_URL + "/students/" + self.stdNum)
	...
	def close_door(self):
		...
		if self.result[0] != -1 and self.result[1] >= 65:
			close_dic = {
				"closing_member": STUDENT_LIST[self.result[0]]
			}
			
			res = requests.post(SERVER_URL + "/close", data=json.dumps(close_dic), headers=headers)
